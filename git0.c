...
#undef F
  ...
  /* Storage layer initialized lazily on first git0_init() or git0_repo()
   * call. The storage prepared statements persist until process exit.
   * Applications using sqlite3_close_v2 handle this cleanly; the sqlite3
   * CLI uses sqlite3_close (v1) which prints a harmless warning. */
